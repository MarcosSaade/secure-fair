  ...
  const StudentRegister = () => {
    ...
    const validateForm = () => {
      ...
      if (!formData.matricula.trim()) {
        ...
        // La matricula debe comenzar con A0
      } else if (!/^A0/.test(formData.matricula)) {
        newErrors.matricula = 'La matrícula debe comenzar con A0 (ej: A01659876)';
      }
  ...
  const handleContinue = () => {
    ...
    const matriculaRepetida = storageService.getEstudiantes().some(est => est.matricula === formData.matricula);
    if (matriculaRepetida){
      setErrors((prev) => ({ ...prev, matricula: "Esta matrícula ya está asociada a otra cuenta" }));
      return;
    }
