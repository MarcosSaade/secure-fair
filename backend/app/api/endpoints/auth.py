...
async def register(registration: RegisterRequest, db: Session = Depends(get_db)):
    ...
    **Security:**
    ...
    if registration.role == "STUDENT" and not registration.major:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="major is required for STUDENT role"
        )

    if registration.role == "STUDENT" and registration.semester is None:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="semester is required for STUDENT role"
        )
    ...
    if user_role == UserRole.STUDENT:
        ...
        existing_student = (
            db.query(Student)
            .filter(Student.student_id_number == registration.student_id_number)
            .first()
        )
        if existing_student:
            db.rollback()
        ...
        org = db.query(Organization).filter(Organization.id == registration.organization_id).first()
        if not org:
            db.rollback()
            raise HTTPException(
...
async def change_password(
    ...
    **Security:**
    ...
    if not pwd_handler.verify_password(
        current_user.password_hash, password_change.current_password
    ):
        raise HTTPException(
