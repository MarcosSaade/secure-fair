...
from app.models.models import (
    Enrollment, Student, TimeSlot, EnrollmentCode, User, UserRole, SlotStatus
)
...
async def create_enrollment(
    ...
    **Process:**
    ...
    time_slot = db.query(TimeSlot).filter(
        TimeSlot.id == enrollment_data.time_slot_id
    ).first()
...
async def get_slot_enrollments(
    ...
    **ADMIN ONLY** - Requires ADMIN role.
    ...
    enrollments = db.query(Enrollment).filter(
        Enrollment.time_slot_id == time_slot_id
    ).all()
...
@router.post(
    "/verify/{enrollment_id}",
    summary="Verify Enrollment Signature"
)
